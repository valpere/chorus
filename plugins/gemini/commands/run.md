---
description: Delegate a task to Gemini CLI non-interactively
argument-hint: "[--background|--wait] <task description>"
disable-model-invocation: true
allowed-tools: Bash(node:*), Bash(git:*), AskUserQuestion
---

Delegate a task to Gemini CLI and return its output verbatim.

Raw slash-command arguments:
`$ARGUMENTS`

Execution mode rules:
- If the raw arguments include `--background`, run Gemini in the background.
- If the raw arguments include `--wait`, run Gemini in the foreground.
- If neither flag is present, ask the user with two options: `Wait for results (Recommended)` or `Run in background`.
- Strip `--background` and `--wait` before passing the task to Gemini.

Pre-flight check:
- Run: `node "$CLAUDE_PLUGIN_ROOT/scripts/companion.mjs" check`
- If it exits non-zero, stop and tell the user to run `/gemini:setup`.
...
```
- Tell the user: "Gemini task started in the background."
