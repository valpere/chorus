---
description: Delegate a task to Kilo Code CLI non-interactively
argument-hint: "[--background|--wait] <task description>"
disable-model-invocation: true
allowed-tools: Bash(node:*), Bash(git:*), AskUserQuestion
---

Delegate a task to Kilo Code CLI and return its output verbatim.
...
- Strip `--background` and `--wait` before passing the task to Kilo.

Pre-flight check:
- Run: `node "$CLAUDE_PLUGIN_ROOT/scripts/companion.mjs" check`
- If it exits non-zero, stop and tell the user to run `/kilo:setup`.
...
```
- Tell the user: "Kilo task started in the background."
